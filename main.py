...
░╚════╝░╚═╝░░╚═╝╚═╝╚═╝░░░░░╚═╝╚═╝░░░░░╚═╝░░╚═╝  ╚═╝░░╚═╝╚═╝
    ...
    print_border("CHIMPX AI", Fore.GREEN)
    print(f"{Fore.YELLOW}│ {'Website'}: {Fore.CYAN}https://app.chimpx.ai/{Style.RESET_ALL}")
...
async def run_autoref(language: str):
    from scripts.autoref import run_autoref as autoref_run
    await autoref_run(language)

async def run_chimpx(language: str):
    from scripts.chimpx import run_chimpx as chimpx_run
    await chimpx_run(language)
...
def get_available_scripts(language):
    scripts = {
        'vi': [
            {"name": "1. Tự động Auto Reff", "value": "autoref"},
            {"name": "2. Tự động nhận token CHIMPX → (pvkey.txt)", "value": "chimpx"},
            {"name": "X. Thoát", "value": "exit"},
        ],
        'en': [
            {"name": "1. Auto Reff", "value": "autoref"},
            {"name": "2. Auto receive CHIMPX tokens → (pvkey.txt)", "value": "chimpx"},
